...
def proper_save(save_dir):
    """Keeps only the latest checkpoint in the main save_dir."""
    
    # Get all checkpoint subfolders
    checkpoint_dirs = [d for d in os.listdir(save_dir) if d.startswith("checkpoint-") and os.path.isdir(os.path.join(save_dir, d))]

    if not checkpoint_dirs:
        print("No checkpoint folders found. Nothing to move.")
        return

    # Sort checkpoints by number (assuming format 'checkpoint-XX')
    checkpoint_dirs.sort(key=lambda x: int(x.split("-")[-1]))
    latest_checkpoint = checkpoint_dirs[-1]  # Get the latest checkpoint

    # Remove non-checkpoint files and folders
    for item in os.listdir(save_dir):
        item_path = os.path.join(save_dir, item)
        if item not in checkpoint_dirs:  # Keep only checkpoint subfolders
            if os.path.isdir(item_path):
                shutil.rmtree(item_path)
            else:
                os.remove(item_path)

    # Move files from the latest checkpoint to the main folder
    latest_checkpoint_path = os.path.join(save_dir, latest_checkpoint)
    for file_name in os.listdir(latest_checkpoint_path):
        src_path = os.path.join(latest_checkpoint_path, file_name)
        dest_path = os.path.join(save_dir, file_name)
        shutil.move(src_path, dest_path)

    # Remove the now-empty checkpoint folder
    os.rmdir(latest_checkpoint_path)

    print(f"Properly saved latest checkpoint: {latest_checkpoint}")
